...
//Enable Config, the events defined are enabled by default (son todo unos)
...
uint8_t PUSService5::GetRIDEnableConfigIndex(uint16_t RID) {
	...
	uint8_t index = 4; //Default: Not valid index.
...
bool PUSService5::IsRIDEnabled(uint16_t RID) {
	...
	//miramos la posicion del RID
	index = GetRIDEnableConfigIndex(RID);

	offset = GetRIDEnableConfigOffset(RID);

	//y comprobamos, con esa info, si en la estructura está activado dicho RID
...
void PUSService5::ExecTC(CDTCHandler &tcHandler, CDTMList &tmList) {
	...
	//habilitamos unicamente los TCs [5,5] y [5,6]
	case (5):
