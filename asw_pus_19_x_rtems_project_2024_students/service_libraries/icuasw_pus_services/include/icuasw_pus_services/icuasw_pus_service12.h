...
protected:
  ...
  static void TriggerEvent(CDEventList & eventList,uint16_t RID, uint16_t PID
 		  	                      , uint16_t PIDValue, uint16_t limit);

  static void UpdateStatus(uint16_t PMONID, enum CheckStatus_t newStatus);
