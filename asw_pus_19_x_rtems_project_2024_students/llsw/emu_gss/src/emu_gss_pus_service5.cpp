...
void GSSServ5TMHandler::ShowTM_5_4(){
	...
	printf(" High Severity Anomaly RID 0x%X ",rid);

	switch (rid){
		case(0x4000):{
			uint16_t pid= GetNextUInt16AppDataField();
			uint16_t pidCurrentValue= GetNextUInt16AppDataField();
			uint16_t limit= GetNextUInt16AppDataField();

			printf(" PID %d = %d ",pid,pidCurrentValue);
			printf(" Low Limit = %d ",limit);
		}
			break;
